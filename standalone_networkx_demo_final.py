...
def query_experts_by_topic(G, topic):
    ...
    if knowledge_node is None:
        print(f"Topic '{topic}' not found in the graph.")
        return []

    # Find documents that cover this topic (incoming edges to knowledge node)
    covering_docs = []
    for edge in G.in_edges(knowledge_node, data=True):
        source, target, data = edge
        if data.get('relationship') == 'COVERS':
            covering_docs.append(source)
            print(f"  Document {source} covers {topic}")

    # Find people who authored these documents (incoming edges to documents)
    for doc_node in covering_docs:
        print(f"  Checking document {doc_node}")
        for edge in G.in_edges(doc_node, data=True):
            source, target, data = edge
            if data.get('relationship') == 'AUTHORED':
                person_data = G.nodes[source]
                experts.append({
                    'id': source,
                ...
                print(f"    {person_data.get('name')} authored {doc_node}")
